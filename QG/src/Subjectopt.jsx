...
function Subjectopt() {
  ...
  // Added missing subjectOptions array
  ...
  useEffect(() => {
    try {
      ...
      const savedOutputData = localStorage.getItem('subjectOptions');
      ...
      if (savedOutputData) {
        const parsedOutput = JSON.parse(savedOutputData);
        if (Array.isArray(parsedOutput)) {
          setSavedOutput(parsedOutput);
          // Only update subjectOptions if we have valid data
          if (parsedOutput.length > 0) {
            setSubjectOptions(parsedOutput);
          }
        }
      }
    } catch (error) {
  ...
  function getRandomWords(text, count) {
    ...
    // Ensure there are words to select
    if (words.length === 0) return [];
    
    // Shuffle the words randomly
    const shuffled = [...words].sort(() => 0.5 - Math.random());
    
    // Return the first `count` words (or fewer if not enough words)
    return shuffled.slice(0, Math.min(count, shuffled.length));
  }
  
  // Handle generation button click
  const handleGenerate = () => {
    // Example text if none is provided
    if (!previewText) {
      const demoText = isArabic 
        ? "هذا مثال للنص العربي الذي يمكن استخدامه للعرض التوضيحي. يحتوي هذا النص على عدة جمل لإظهار كيفية عمل التطبيق."
        : "This is an example text that can be used for demonstration. This text contains several sentences to show how the application works.";
      
      setPreviewText(demoText);
      localStorage.setItem("subjectText", demoText);
    }
    
    // Update i3rabWords if needed
    if (i3rabCount > 0) {
      const selectedWords = getRandomWords(previewText, i3rabCount);
      setI3rabWords(selectedWords);
    }
            ...
            {previewText && (
              <>
                {isArabic ? (
                  ...
                  <h3 className="font-bold text-lg mt-6 mb-2">Reading comprehension:</h3>
                )}
                ...
                {isArabic ? (
                  ...
                  <>
                    ...
                    <h3 className="font-bold text-lg mb-1">2- Answer the following questions:</h3>
                ...
                {isArabic ? (
                  ...
                  <h3 className="font-bold text-lg mt-6 mb-2">Grammar:</h3>
                )}
                ...
                {i3rabWords.length > 0 && (
                  <>
                    {isArabic ? (
                      ...
                      <h3 className="font-bold text-lg mt-6 mb-2">1- Parse the following words:</h3>
                    )}
                ...
                {parsedWords.length > 0 && (
                  <>
                    {isArabic ? (
                      ...
                      <h3 className="font-bold text-lg mt-6 mb-2">2- Complete the following table:</h3>
                    )}
                    <table className="w-full border border-gray-300 text-sm mt-2">
                      <thead>
                        <tr className="bg-gray-100">
